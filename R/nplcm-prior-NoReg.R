...
nplcm_NoReg_prior <- function(data_nplcm, model_options, mcmc_options) {
    ...
    # model_options:
    dput(model_options, 
         file.path(mcmc_options$result.folder, "model_options.txt"))
    # mcmc_options:
    dput(mcmc_options, 
         file.path(mcmc_options$result.folder, "mcmc_options.txt"))
    ...
    if ("BrS" %in% use_measurements){
      ...
      for (s in seq_along(template_BrS_list)) {
        if (sum(template_BrS_list[[s]]) == 0) {
          warning(paste0(
              "==[baker] Bronze-standard slice ",
              names(data_nplcm$Mobs$MBS)[s], 
      ...
      for(i in seq_along(JBrS_list)) {
        ...
        assign(paste("templateBS", i, sep = "_"),
                     as.matrix_or_vec(template_BrS_list[[i]]))   
      }
      ...
      if (parsed_model$BrS_grp) {
        BrS_TPR_strat <- TRUE
        for(i in seq_along(JBrS_list)) {
          assign(paste("GBrS_TPR", i, sep = "_"),
                 length(unique(prior_BrS$grp)))    
          assign(paste("BrS_TPR_grp", i, sep = "_"),
                 prior_BrS$grp)
        }
      }
      ...
      for (i in seq_along(JBrS_list)) {
        if (!is.null(prior_BrS$grp)) {  # <--- need to change to list if we have multiple slices.
          assign(paste("GBrS_TPR", i, sep = "_"),
                 length(unique(prior_BrS$grp)))  # <--- need to change to depending on i if grp change wrt specimen.
        }
        if (is.null(prior_BrS$grp)) { # <--- need to change to list if we have multiple slices.
      ...
      for(i in seq_along(JBrS_list)) {
        if (likelihood$k_subclass[i] == 1) {
          BrS_tpr_prior <- set_prior_tpr_BrS_NoNest(
                               i, model_options, data_nplcm)
        }
        if (likelihood$k_subclass[i] > 1) {
          BrS_tpr_prior <- set_prior_tpr_BrS_NoNest(
                               i, model_options, data_nplcm)
        ...
        alpha_mat[[i]] <- matrix(NA, nrow = GBrS_TPR_curr,
                                 ncol = JBrS_list[[i]])
        beta_mat[[i]] <- matrix(NA, nrow = GBrS_TPR_curr,
                                ncol = JBrS_list[[i]])
      ...
      if (!BrS_TPR_strat) {
        ...
        if (length(single_column_MBS) == 0) {
          ...
          in_data <- c(in_data, "Nd", "Nu", "Jcause", "alphaEti",
                       paste("JBrS",
                             1:length(JBrS_list),
                             sep="_")[- single_column_MBS],  # <---- no need to iterate in .bug file for a slice with one column.
                       paste("MBS", 1:length(JBrS_list), sep = "_"),
                       paste("templateBS", 1:length(JBrS_list), sep = "_")
                       # paste("alphaB",1:length(JBrS_list),sep="_"),
                       # paste("betaB",1:length(JBrS_list),sep="_")
      ...
      for (s in seq_along(Mobs$MBS)) {
        ...
        if (likelihood$k_subclass[s] == 1) {
          ...
          out_parameter <- unique(c(out_parameter,
                                    paste(c("ThetaBS", "PsiBS", "Lambda",
                                            "Eta","alphadp0","alphadp0_case"),
                                          s, sep = "_")))
        }
      }
      #
        ...
        for (s in seq_along(Mobs$MBS)) {
          ...
          if (likelihood$k_subclass[s] == 1) {
            ...
            if (GBrS_TPR_curr == 1) {
              ...
              res_curr[[1]] <-
                  matrix(stats::rbeta(GBrS_TPR_curr*JBrS_list[[s]], 1, 1),
          ...
          if (likelihood$k_subclass[s] > 1) {
            ...
            names(res_curr) <-
                paste(c("r0", "r1", "alphadp0", "alphadp0_case"), s, sep = "_")
            res <- c(res,res_curr)
          }
        }
        res
      ...
      in_init <- function() {
        ...
        for(i in seq_along(JSS_list)) {
          ...
          if (GSS_TPR_curr == 1) {
            ...
            tmp_thetaSS[[i]] <- 
                matrix(stats::rbeta(GSS_TPR_curr*JSS_list[[i]], 1, 1),
          ...
          if (i == 1){
            ...
            #}  
            tmp_Icat_case[[i]] <-
                init_latent_jags_multipleSS(MSS_list, likelihood$cause_list)
          }
        }
        res <- c(tmp_thetaSS, tmp_Icat_case)
        names(res) <-
            c(paste("thetaSS", seq_along(JSS_list), sep = "_"), "Icat")
        res
      }
    } 
    ...
    # if (!is.null(mcmc_options$individual.pred) &&
    #     mcmc_options$individual.pred) {
    #   out_parameter <- c(out_parameter, "Icat")
    # }
    # get posterior predictive distribution of BrS measurments:
    # if (!is.null(mcmc_options$ppd) && mcmc_options$ppd) {
    #   out_parameter <- 
    #       c(out_parameter,
    #         paste("MBS.new", seq_along(Mobs$MBS), sep = "_"))
    # }
    #
